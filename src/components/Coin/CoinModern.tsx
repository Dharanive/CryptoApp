...
import { TrendingUp, TrendingDown, Star, BarChart3, Globe, Calendar } from 'lucide-react'
import { cn } from '@/lib/utils'

interface CoinModernProps {
  coinId: string;
}
...
const CoinModern: React.FC<CoinModernProps> = ({ coinId }) => {
  const [coinData, setCoinData] = useState<CoinData | null>(null);
  const [historicalData, setHistoricalData] = useState<HistoricalData | null>(null);

  const context = useContext(CoinContext);
  if (!context) {
    throw new Error('CoinModern must be used within CoinContextProvider');
  }
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Array.from({ length: 4 }).map((_, i) => (
                <Card key={i} className="bg-slate-900/30 border-slate-700">
